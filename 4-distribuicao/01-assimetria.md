...
- Gráfico que mostra se os dados seguem uma distribuição escolhida
- O eixo Y são os dados reais e o eixo X os valores esperados caso eles sigam a distribuição escolhida
- Caso os dados sigam a distribuição, eles formarão uma linha reta (geralmente tem uma linha reta de base para comparar)
- Caso o formato seja outro, indica que não é essa a distribuição correta
- Ela não dá numericamente se os dados são dessa distribuição, é só visual
- Serve para diversas distribuições

![](images/assimetria/qq-plot.png)
